...
export default function BOSuporte() {
  ...
  return <div className="space-y-5">
    ...
    {loading ? <Loader2 className="mx-auto my-16 h-6 w-6 animate-spin" /> :
      <div className="grid gap-4 xl:grid-cols-4">{columns.map((column) => <section key={column.status} className="rounded-lg border bg-muted/35">
        ...
        <div className="space-y-3 p-3">{grouped[column.status].map((ticket) => {
          ...
          return <article key={ticket.id} className="space-y-3 rounded-md border bg-white p-3 shadow-sm">
            <div className="flex items-start justify-between gap-2"><span className="text-xs font-medium text-primary">{ticket.ticketNumber}</span><Badge variant={ticket.priority === "critical" ? "destructive" : "outline"}>{ticket.priority}</Badge></div>
            ...
            {ticket.slaDeadline && <p className={`flex items-center gap-1 text-xs ${overdue ? "text-destructive" : "text-muted-foreground"}`}><Clock3 className="h-3 w-3" />{overdue ? "SLA vencido" : new Date(ticket.slaDeadline).toLocaleString("pt-BR")}</p>}
