...
interface RawSprint    { id: string; name: string; end_date: string | null; is_active: boolean; team_id: string; }
    ...
    async function load() {
      ...
      try {
        const [huRes, spRes, impRes, demRes] = await Promise.all([
          ...
          supabase.from("sprints").select("id, name, end_date, is_active, team_id").limit(200),
