...
function DossierComboboxContent({ searchPlaceholder, emptyMessage, listTestId, children }: { searchPlaceholder: string; emptyMessage: string; listTestId?: string; children: ReactNode }) {
  return <PopoverContent
    data-testid={listTestId ? `${listTestId}-content` : undefined}
    side="bottom"
    align="start"
    sideOffset={4}
    avoidCollisions
    ...
    className="z-50 min-w-[var(--radix-popover-trigger-width)] max-w-[calc(100vw-1rem)] overflow-hidden p-0"
    style={{ width: "var(--radix-popover-trigger-width)", maxHeight: "var(--radix-popover-content-available-height)" }}
  >
    <Command className="min-h-0 min-w-0">
      <CommandInput className="shrink-0" placeholder={searchPlaceholder} />
      <CommandList
        data-testid={listTestId}
        className="max-h-[min(20rem,calc(var(--radix-popover-content-available-height)-3rem))] overflow-x-hidden overflow-y-auto overscroll-contain pb-1 pr-1 [scrollbar-gutter:stable]"
      >
        <CommandEmpty>{emptyMessage}</CommandEmpty>
        <CommandGroup className="min-w-0">{children}</CommandGroup>
      </CommandList>
    </Command>
  </PopoverContent>;
}
...
function ProjectCombobox({ id, value, options, onChange, currentTeamId }: { id: string; value: string; options: ApfDossierCreationProject[]; onChange: (value: string) => void; currentTeamId: string | null }) {
  ...
  return <Popover open={open} onOpenChange={setOpen}><PopoverTrigger asChild><Button id={id} type="button" variant="outline" role="combobox" aria-expanded={open} className="min-h-11 w-full min-w-0 justify-between overflow-hidden px-3 font-normal"><span title={selected ? `${selected.code ? `${selected.code} · ` : ""}${selected.name}` : undefined} className={cn("flex min-w-0 items-center gap-1.5 text-left", !selected && "text-muted-foreground")}>{selected ? <>{selected.code && <span className="shrink-0 font-mono text-xs text-muted-foreground">{selected.code}</span>}<span className="min-w-0 truncate">{selected.name}</span></> : "Selecione ou busque um projeto"}</span><ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" aria-hidden="true" /></Button></PopoverTrigger><DossierComboboxContent searchPlaceholder="Buscar por projeto, código ou contrato…" emptyMessage="Nenhum projeto encontrado." listTestId="dossier-project-list">{orderedOptions.map((project) => { const isCurrentTeam = project.teamIds.includes(currentTeamId ?? ""); const fullLabel = `${project.code ? `${project.code} · ` : ""}${project.name}`; return <CommandItem key={project.id} value={`${project.code ?? ""} ${project.name} ${project.contractName}`} onSelect={() => { onChange(project.id); setOpen(false); }} className={cn("min-w-0 items-start gap-2 py-3 pr-2", value === project.id && "bg-accent/60")}><Check className={cn("mt-0.5 h-4 w-4 shrink-0", value === project.id ? "opacity-100" : "opacity-0")} aria-hidden="true" /><FolderKanban className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" aria-hidden="true" /><span className="grid min-w-0 flex-1"><span title={fullLabel} className="flex min-w-0 items-baseline gap-1.5"><span className="shrink-0 font-mono text-xs text-muted-foreground">{project.code ?? "—"}</span><span className="min-w-0 truncate font-medium">{project.name}</span></span><span title={project.contractName} className="truncate text-xs text-muted-foreground">{project.contractName}{isCurrentTeam ? " · Projeto do time ativo" : ""}</span></span></CommandItem>; })}</DossierComboboxContent></Popover>;
}
...
function StoryCombobox({ id, value, options, onChange, disabled }: { id: string; value: string; options: ApfDossierCreationUserStory[]; onChange: (value: string) => void; disabled: boolean }) {
  const [open, setOpen] = useState(false);
  const selected = options.find((item) => item.id === value);
  return <Popover open={open} onOpenChange={setOpen}><PopoverTrigger asChild><Button id={id} type="button" variant="outline" role="combobox" aria-expanded={open} disabled={disabled} className="min-h-11 w-full min-w-0 justify-between overflow-hidden px-3 font-normal"><span className={cn("min-w-0 truncate text-left", !selected && "text-muted-foreground")}>{selected ? `${selected.code} · ${selected.title}` : "Selecione ou busque uma história"}</span><ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" aria-hidden="true" /></Button></PopoverTrigger><DossierComboboxContent searchPlaceholder="Buscar por código ou título…" emptyMessage="Nenhuma história encontrada." listTestId="dossier-story-list">{options.map((story) => <CommandItem key={story.id} value={`${story.code} ${story.title}`} onSelect={() => { onChange(story.id); setOpen(false); }} className={cn("min-w-0 items-start gap-2 py-3 pr-2", value === story.id && "bg-accent/60")}><Check className={cn("mt-0.5 h-4 w-4 shrink-0", value === story.id ? "opacity-100" : "opacity-0")} aria-hidden="true" /><span className="grid min-w-0"><span title={`${story.code} · ${story.title}`} className="truncate font-medium">{story.code} · {story.title}</span><span className="text-xs text-muted-foreground">{formatApfStatus(story.status)}</span></span></CommandItem>)}</DossierComboboxContent></Popover>;
}
...
function SessionCombobox({ id, value, options, onChange, disabled }: { id: string; value: string; options: ApfDossierCreationSession[]; onChange: (value: string) => void; disabled: boolean }) {
  ...
  return <Popover open={open} onOpenChange={setOpen}><PopoverTrigger asChild><Button id={id} type="button" variant="outline" role="combobox" aria-expanded={open} disabled={disabled} className="min-h-11 w-full min-w-0 justify-between overflow-hidden px-3 font-normal"><span className="min-w-0 truncate text-left">{selectedLabel?.label ?? "Vincular posteriormente"}</span><ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" aria-hidden="true" /></Button></PopoverTrigger><DossierComboboxContent searchPlaceholder="Buscar modelo, baseline, sprint ou release…" emptyMessage="Nenhuma sessão encontrada."><CommandItem value="vincular posteriormente" onSelect={() => { onChange("none"); setOpen(false); }} className={cn("min-w-0 items-start gap-2 py-3 pr-2", value === "none" && "bg-accent/60")}><Check className={cn("mt-0.5 h-4 w-4 shrink-0", value === "none" ? "opacity-100" : "opacity-0")} aria-hidden="true" /><span className="grid min-w-0"><span className="font-medium">Vincular posteriormente</span><span className="text-xs text-muted-foreground">Crie o dossiê agora e associe uma contagem depois.</span></span></CommandItem>{options.map((session) => { const option = formatApfSessionOption(session); return <CommandItem key={session.id} value={`${option.label} ${option.description}`} onSelect={() => { onChange(session.id); setOpen(false); }} className={cn("min-w-0 items-start gap-2 py-3 pr-2", value === session.id && "bg-accent/60")}><Check className={cn("mt-0.5 h-4 w-4 shrink-0", value === session.id ? "opacity-100" : "opacity-0")} aria-hidden="true" /><span className="grid min-w-0"><span title={option.label} className="truncate font-medium">{option.label}</span><span className="text-xs text-muted-foreground">{option.description}</span></span></CommandItem>; })}</DossierComboboxContent></Popover>;
}
