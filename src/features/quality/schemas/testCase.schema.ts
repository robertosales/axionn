...
export const qualityTestCaseUpdateSchema = z.object({
  organizationId: z.string().uuid(),
  contractId: optionalUuid,
  projectId: optionalUuid,
  teamId: optionalUuid,
  ...
  postconditions: z.string().max(20_000).nullable().optional(),
  testData: z.string().max(20_000).nullable().optional(),
  testType: z.enum(["functional", "regression", "integration", "api", "security", "accessibility", "compatibility", "usability", "performance", "uat", "other"]).optional(),
  priority: z.enum(["low", "medium", "high", "critical"]).optional(),
  severity: z.enum(["low", "medium", "high", "critical"]).optional(),
  status: z.enum(["draft", "ready", "approved", "deprecated", "archived"]).optional(),
  executionMode: z.enum(["manual", "automated", "hybrid"]).optional(),
  estimatedMinutes: z.number().int().nonnegative().nullable().optional(),
  tags: z.array(z.string().trim().min(1).max(80)).max(50).optional(),
  steps: z.array(qualityTestStepSchema).min(1).max(200).optional(),
});

export type QualityTestCaseUpdatePayload = z.infer<typeof qualityTestCaseUpdateSchema>;
