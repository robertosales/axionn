 ...
 * Sem alterações de lógica — apenas refatoração visual.
 */
...
import { CheckCircle2, XCircle, RefreshCw, PlusCircle, MinusCircle, Clock, Loader2 } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { cn } from "@/lib/utils";

export interface PreviewRow {
  rhm: string; projeto: string; teamId: string; situacao?: string; tipo: string;
  sla?: string; descricao?: string; data_previsao_encerramento?: string;
  prazo_inicio_atendimento?: string; prazo_solucao?: string;
  tipo_defeito?: string; originada_diagnostico?: boolean;
}

export type RowStatus = "pendente" | "validando" | "atualizando" | "criado" | "atualizado" | "ignorado" | "erro";
...
interface EnrichedRow extends PreviewRow {
  situacaoSistema: string | null; tipoAcao: TipoAcao; diferenca: string | null; status: RowStatus;
}

const SITUACAO_LABELS: Record<string, string> = {
  fila_atendimento: "Fila Atendimento", planejamento_elaboracao: "Em Elaboração",
  planejamento_ag_aprovacao: "Ag. Aprovação", planejamento_aprovada: "Aprovada p/ Exec",
  em_execucao: "Em Execução", bloqueada: "Bloqueada", hom_ag_homologacao: "Ag. Homologação",
  hom_homologada: "Homologada", rejeitada: "Rejeitada", fila_producao: "Fila Produção",
  ag_aceite_final: "Ag. Aceite Final", cancelada: "Cancelada",
};
...
const TIPO_ACAO_CONFIG: Record<TipoAcao, { label: string; icon: React.ElementType; pill: string }> = {
...
const ROW_STATUS_CONFIG: Record<RowStatus, { label: string; icon: React.ElementType; cls: string }> = {
...
export function ImportacaoPreviewTable({ rows, onConfirm, onCancel, loading, progressMap = new Map() }: Props) {
  ...
  useEffect(() => {
    ...
    async function enrich() {
      ...
      try {
        ...
        for (const row of rows) { const list = byTeam.get(row.teamId) ?? []; list.push(row.rhm); byTeam.set(row.teamId, list); }
        const systemMap = new Map<string, SystemRecord>();
        for (const [teamId, rhms] of byTeam) {
          const { data } = await supabase.from("demandas" as any).select("rhm, situacao").eq("team_id", teamId).in("rhm", rhms);
          if (data) for (const d of data as any[]) systemMap.set(`${teamId}:${d.rhm}`, { situacao: d.situacao });
        }
        const result: EnrichedRow[] = rows.map((row) => {
          const sys = systemMap.get(`${row.teamId}:${row.rhm}`) ?? null;
          let tipoAcao: TipoAcao; let diferenca: string | null = null;
          if (!sys) { tipoAcao = "novo"; }
          else if (sys.situacao !== row.situacao) { tipoAcao = "atualizacao"; diferenca = `Situação: "${labelSituacao(sys.situacao)}" → "${labelSituacao(row.situacao)}"`; }
          else { tipoAcao = "sem_alteracao"; }
          return { ...row, situacaoSistema: sys?.situacao ?? null, tipoAcao, diferenca, status: "pendente" };
        });
        setEnriched(result);
        setSelectedRhms(new Set(result.filter((r) => r.tipoAcao === "novo" || r.tipoAcao === "atualizacao").map((r) => r.rhm)));
  ...
  const selectableRhms = useMemo(() => displayRows.filter((r) => r.tipoAcao !== "erro_validacao").map((r) => r.rhm), [displayRows]);
  const allSelected  = selectableRhms.length > 0 && selectableRhms.every((rhm) => selectedRhms.has(rhm));
  const someSelected = selectedRhms.size > 0 && !allSelected;

  function toggleAll() { setSelectedRhms(allSelected ? new Set() : new Set(selectableRhms)); }
  function toggleRow(rhm: string) {
    setSelectedRhms((prev) => { const next = new Set(prev); if (next.has(rhm)) next.delete(rhm); else next.add(rhm); return next; });
  }
  function handleMigrarSelecionados() { onConfirm(enriched.filter((r) => selectedRhms.has(r.rhm))); }
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50 border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <TableHead className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider py-3">#</TableHead>
                  <TableHead className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Projeto</TableHead>
                  <TableHead className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Status Planilha</TableHead>
                  <TableHead className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Status Sistema</TableHead>
                  <TableHead className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Diferença</TableHead>
                  <TableHead className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Áção</TableHead>
                  <TableHead className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Progresso</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {displayRows.map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className="font-mono text-sm font-semibold text-gray-700 py-3.5">#{row.rhm}</TableCell>
                      <TableCell className="text-sm text-gray-600 max-w-[140px] truncate py-3.5" title={row.projeto}>{row.projeto}</TableCell>
                      <TableCell className="py-3.5">
                        <span className="inline-flex items-center text-xs px-2.5 py-1 rounded-full bg-amber-50 border border-amber-200 text-amber-700 font-medium whitespace-nowrap">
                          {labelSituacao(row.situacao)}
                        </span>
                      </TableCell>
                      <TableCell className="py-3.5">
                        {row.situacaoSistema
                          ? <span className="inline-flex items-center text-xs px-2.5 py-1 rounded-full bg-slate-50 border border-slate-200 text-slate-600 font-medium whitespace-nowrap">{labelSituacao(row.situacaoSistema)}</span>
                      ...
                      <TableCell className="py-3.5">
                        <Badge variant="outline" className={cn("text-[11px] gap-1 px-2.5 py-1 rounded-full border font-medium", acfg.pill)}>
                          <AIcon className="h-3 w-3 shrink-0" />
                          {acfg.label}
                        </Badge>
                      </TableCell>
                      <TableCell className="py-3.5">
                        <span className={cn("flex items-center gap-1.5 text-xs font-medium whitespace-nowrap", scfg.cls)}>
...
function Counter({ label, count, cls }: { label: string; count: number; cls: string }) {
  return (
    <div className={cn("rounded-xl border px-4 py-4 text-center", cls)}>
