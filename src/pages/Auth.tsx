...
const Auth = () => {
  ...
  const handleGoogleLogin = async () => {
    if (!PUBLIC_OAUTH_ENABLED) return;
  ...
  const handleForgotPassword = async () => {
    ...
    setLoading(true);
    const { allowed, retryAfter, unavailable } = await checkAuthRateLimit("reset_password", email);
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 p-4">
      <Card className="w-full max-w-md rounded-2xl shadow-lg">
        <CardHeader className="space-y-3 px-8 pb-2 pt-8 text-center">
          ...
          <CardDescription className="text-sm">{APP_TAGLINE}</CardDescription>
        </CardHeader>
        ...
        <CardContent className="space-y-5 px-8 pb-8 pt-4">
          {idleTimeout && (
            <div className="flex items-start gap-3 rounded-xl border border-amber-400/40 bg-amber-50 px-4 py-3">
              ...
              <p className="text-sm text-amber-700">
                Sua sessão foi encerrada por <strong>inatividade</strong>. Faça
          ...
          <div className="flex items-start gap-3 rounded-xl border border-teal-200 bg-teal-50 px-4 py-3">
            ...
            <p className="text-sm text-teal-800">
              O acesso ao Axion é provisionado pela empresa contratante. Entre
              com uma conta previamente cadastrada ou convidada.
            </p>
          </div>
