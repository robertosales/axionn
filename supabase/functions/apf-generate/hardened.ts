...
// Capture the existing implementation without duplicating its large APF pipeline.
...
originalServe(async (request: Request, info: Deno.ServeHandlerInfo) => {
  ...
  // Browser callers cannot inject credentials or provider overrides.
  ...
  // The legacy handler receives service-role authority only after the wrapper
  // has confirmed platform_user_roles.platform_admin.
