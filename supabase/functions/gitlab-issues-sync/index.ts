...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: integration, error: fetchError } = await supabase
      .from("git_integrations").select("*").eq("id", integrationId).eq("provider", "gitlab").single();
    if (fetchError || !integration) return json({ error: "Integration not found" }, 404);
    if (!integration.sync_issues_as_backlog) return json({ error: "sync_issues_as_backlog is disabled for this integration" }, 422);
    if (!integration.team_id) return json({ error: "team_id is required to sync issues as backlog" }, 422);
    if (!integration.repository_path || !integration.access_token_encrypted) return json({ error: "Repository path and access token are required" }, 422);
    ...
    if (!listRes.ok) { const detail = (await listRes.text()).slice(0, 300); return json({ error: "Failed to list GitLab issues", gitlab_status: listRes.status, detail }, 502); }
...
async function upsertHuFromIssue(
  supabase: any, integration: any, issue: Record<string, unknown>,
  payloadLabels: Array<Record<string, unknown>>, correlationId: string,
  ...
  if (huError || !hu) { console.error("[gitlab-issues-sync] Failed to create HU", String(issueId), huError); return "skipped"; }
