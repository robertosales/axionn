...
-- Erro: SELECT DISTINCT + ORDER BY d.created_at incompatível.
-- Solução: subquery para deduplicar IDs antes do loop.
-- ============================================================
...
BEGIN
  FOR v_demanda IN
    ...
    -- Deduplicação via GROUP BY em vez de DISTINCT + ORDER BY
    GROUP BY d.id, d.titulo, d.situacao, d.sla, d.team_id,
             d.project_id, t.project_id,
             d.contract_id, t.contract_id, proj.contract_id, crt.contract_id,
             d.created_at
