...
DECLARE
  ...
  -- Status que pausam o SLA — não contam como WIP ativo
  ...
  FOREACH v_team_id IN ARRAY p_team_ids LOOP
    ...
    SELECT COALESCE(jsonb_agg(
      jsonb_build_object(
        ...
        -- Horas alocadas = soma de total_horas das demandas ATIVAS (não fechadas, não pausadas)
        'allocatedHours', (
        ...
        -- WIP ativo = exclui demandas com SLA pausado
        'wipCount', (
          SELECT COUNT(DISTINCT dr.demanda_id)
          FROM   public.demanda_responsaveis dr
          JOIN   public.demandas d ON d.id = dr.demanda_id
        ...
        -- Demandas pausadas (SLA congelado) — meramente informativo
        'pausedCount', (
          SELECT COUNT(DISTINCT dr.demanda_id)
          FROM   public.demanda_responsaveis dr
          JOIN   public.demandas d ON d.id = dr.demanda_id
        ...
        -- SLA crítico: demandas ativas (não fechadas, não pausadas) cujo prazo_solucao
        -- está a 24h ou menos do agora, OU já estourado.
        'slaCriticalCount', (
          SELECT COUNT(DISTINCT dr.demanda_id)
          FROM   public.demanda_responsaveis dr
          JOIN   public.demandas d ON d.id = dr.demanda_id
        ...
        'realizedHours', (
          SELECT COALESCE(SUM(dh.horas), 0)::numeric
          FROM   public.demanda_hours dh
          JOIN   public.demandas d ON d.id = dh.demanda_id
          WHERE  dh.user_id = p.user_id
            AND  d.team_id  = v_team_id
            AND  dh.data_lancamento >= v_week_start::date
            AND  dh.data_lancamento <  v_week_end::date
        )
      )
    ), '[]'::jsonb)
    INTO v_devs
