...
BEGIN
  ...
  VALUES (
    p_contract_id,
    'PFS/DPF - GlobalWeb',
    'Modelo de contagem PFS baseado no Guia de Métricas DPF / GlobalWeb. Padrão IFPUG simplificado com tipos TRN e ARQ e 34 fatores de impacto contratuais.',
    'pfs_dpf'
  )
  ...
  INSERT INTO public.apf_counting_rules (
    ...
    v_model_id,
    'Executar a contagem de PFS da sprint a partir de: Baseline homologado do sistema, Histórias de Usuário (HUs) da sprint e Evidências técnicas (SQL, Swagger, telas, logs). Calcular PF Bruto e PF FS. Gerar Documento de Evidência de Contagem. Registrar zonas cinzentas, precedentes e decisões do time. Reproduzir o padrão efetivamente adotado pela equipe de métricas na medição oficial do contrato.',
    'A unidade de contagem é a EF do baseline do sistema, mas a contagem deve respeitar a unicidade do processo elementar. A HU nunca é unidade de contagem — é apenas gatilho de impacto. Se uma EF do baseline foi impactada, ela deve ser avaliada. Porém, só deve ser contada como item separado quando houver um processo elementar único, completo e independente. Consultas explícitas, validações, histórico, preview e ações auxiliares não devem ser separadas automaticamente; elas só contam à parte se o time oficialmente as tratar como processos elementares distintos.',
    '1. Medição oficial homologada do contrato e histórico da equipe. 2. Baseline homologado do sistema. 3. Contagens e evidências homologadas anteriores. 4. Precedentes históricos do time. 5. Este prompt/regras do modelo. 6. Casos análogos. 7. Artefatos da sprint. Nunca usar a teoria do prompt para contrariar o padrão historicamente adotado pelo time.',
    'Baseline impactado não significa multiplicação automática de itens. O processo central da funcionalidade deve ser identificado primeiro. Consultas explícitas, histórico, preview, validações e ações auxiliares podem ser parte do mesmo processo elementar. Só separar em itens distintos quando houver unicidade funcional clara. Não consolidar funções distintas se o time historicamente as mede separadas. Não inventar processos elementares não reconhecidos pelo baseline ou precedente. Situações iguais devem ter decisões iguais. Se a planilha oficial contou de um modo, esse modo prevalece. O objetivo não é maximizar PF — é reproduzir a contagem oficial do contrato.',
    'Identificar primeiro o objetivo funcional principal da HU (processo central). Consultas explícitas não são automaticamente novas funções — só contam separadamente se forem processos elementares únicos, completos e independentes. Histórico, preview, validação, atualizar painel, copiar, carregar, testar e ações semelhantes não devem ser separadas por padrão. Critério de unicidade: tem objetivo funcional próprio + percebida pelo usuário como completa e independente + não é etapa do fluxo principal + há precedente do baseline ou da medição oficial para contá-la separadamente.',
    'Cada EF do baseline impactada deve ser avaliada. O resultado pode ser uma única linha de contagem por processo elementar. Mesmo código, mesma HU e mesma regra não devem ser multiplicados automaticamente. Não colapsar funções distintas que o time mede separadamente. Não abrir funções novas só porque há mais de uma ação na tela. Se uma mesma EF aparecer em HUs diferentes, seguir o precedente oficial do contrato.',
    'Em conflito entre rigor teórico e histórico do time, vence o histórico do time. A contagem deve refletir como o time realmente mede no contrato. Não aplicar automaticamente A75 apenas porque o guia teórico permite. Se o histórico oficial usar A = 60%, usar A = 60%. Se o histórico oficial usar A75 = 75%, usar A75 = 75%.',
    'Antes de medir, verificar se a sprint pertence integralmente ao escopo da OS. Se houver arquivo parcial, recorte de time, lote ou sprint, não ampliar escopo por inferência. Se a medição oficial já mostrou como classificar uma EF, reutilizar o mesmo critério. Não forçar harmonização com o guia teórico quando o contrato já revela o padrão operacional real.',
    'Quando houver dúvida entre separar ou consolidar processos explícitos, prevalece a consolidação. Consultas explícitas e ações auxiliares só viram itens separados se houver precedente claro. O objetivo é evitar supercontagem por fragmentação excessiva. A métrica deve refletir o processo central realmente reconhecido pelo time.'
  );
