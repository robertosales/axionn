...
-- Configurações editáveis da organização e trilha de auditoria.
...
begin
  ...
  if char_length(v_name) < 2 or char_length(v_name) > 120 then
    raise exception using
      errcode = '22023',
      message = 'organization_settings_invalid_name';
  end if;

  if v_contact_name is not null and char_length(v_contact_name) > 120 then
    raise exception using
      errcode = '22023',
      message = 'organization_settings_invalid_contact_name';
  end if;
  ...
  if v_contact_email is not null
    ...
    raise exception using
      errcode = '22023',
      message = 'organization_settings_invalid_contact_email';
  end if;
  ...
  if v_logo_url is not null
    ...
    raise exception using
      errcode = '22023',
      message = 'organization_settings_invalid_logo_url';
  end if;
...
comment on table public.organization_settings_audit_log is
  'Trilha de auditoria das configurações editáveis da organização.';
comment on function public.update_organization_settings_v2(uuid,text,text,text,text) is
  'Atualiza apenas nome, contato e logo da organização ativa, registrando auditoria.';
