...
begin
 if not public.can_manage_quality(p_org_id) then raise exception using errcode='42501',message='quality_unlink_denied'; end if;
...
declare v_id uuid;
begin
 if not public.can_manage_quality(p_org_id) then raise exception using errcode='42501',message='quality_suite_create_denied'; end if;
...
declare v_id uuid;
begin
 if not public.can_manage_quality(p_org_id) then raise exception using errcode='42501',message='quality_suite_item_denied'; end if;
