...
declare v_id uuid;
begin
 if not public.can_quality_permission_v1(p_org_id,'manage_test_plans') then raise exception using errcode='42501',message='quality_plan_create_denied'; end if;
...
language plpgsql security definer set search_path=public,pg_temp as $$
begin
 if not public.can_quality_permission_v1(p_org_id,'manage_test_plans') then raise exception using errcode='42501',message='quality_plan_update_denied'; end if;
 perform public.assert_quality_scope(p_org_id,(p_payload->>'contract_id')::uuid,(p_payload->>'project_id')::uuid,(p_payload->>'team_id')::uuid);
 if nullif(btrim(p_payload->>'name'),'') is null then raise exception using errcode='22023',message='quality_plan_name_required'; end if;
...
begin
 if not public.can_quality_permission_v1(p_org_id,'manage_test_plans') then raise exception using errcode='42501',message='quality_plan_item_denied'; end if;
