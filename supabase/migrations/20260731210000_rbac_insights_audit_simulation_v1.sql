-- RBAC insights v1: auditoria tenant-scoped e simulação de acesso efetivo.
-- As funções são somente leitura e reservadas a administradores da organização.
...
comment on function public.list_rbac_audit_events_v1(uuid, integer, text) is
  'Lista eventos RBAC do tenant para administradores, com filtro opcional por perfil.';
comment on function public.simulate_rbac_user_access_v1(uuid, uuid) is
  'Simula os perfis e permissões efetivas por módulo de um membro da organização.';
