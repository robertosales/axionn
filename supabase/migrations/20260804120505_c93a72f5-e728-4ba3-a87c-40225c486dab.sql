...
begin
  ...
  -- Fallback: quando o briefing nao tem sprint, usa a sprint ativa da equipe
  v_sprint_id := v_briefing.sprint_id;
  if v_sprint_id is null then
    select sprint.id into v_sprint_id
    from public.sprints sprint
    where sprint.team_id = v_briefing.team_id
      and sprint.closed_at is null
    order by sprint.is_active desc nulls last, sprint.start_date desc nulls last, sprint.created_at desc
    limit 1;
  end if;
...
revoke all on function public.apply_ai_briefing_suggestion(uuid) from public, anon;
grant execute on function public.apply_ai_briefing_suggestion(uuid) to authenticated;