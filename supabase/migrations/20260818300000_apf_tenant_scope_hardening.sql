-- APF tenant scope hardening + attachments storage ownership scoping.
-- Idempotent: safe to re-run. Mirrors the fixes applied for security findings
-- apf_counting_models_null_org_exposure and attachments_bucket_select_policy_no_org_scope.

-- 1) Remove the "contract.org_id is null" cross-tenant bypass from APF access helpers.
...
returns boolean language sql stable security definer set search_path to 'public','pg_temp' as $function$
  select exists (
    ...
    join public.contracts contract on contract.id = model.contract_id
    where model.id = _model_id
      and contract.org_id = any(public.my_org_ids())
  );
$function$;
...
returns boolean language sql stable security definer set search_path to 'public','pg_temp' as $function$
  select exists (
    ...
    join public.contracts contract on contract.id = project.contract_id
    where baseline.id = _baseline_id
      and contract.org_id = any(public.my_org_ids())
  );
$function$;

-- 2) Recreate the APF policies without the null-org bypass.
...
-- 3) Attachment object reads require team membership, not just a file_path match.
  ...
  and (
    ...
    or exists (
      ...
      where a.file_path = storage.objects.name
        and public.can_view_team(auth.uid(), a.team_id)
    )
    or exists (
      select 1 from public.demanda_evidencias e
      join public.demandas d on d.id = e.demanda_id
      where e.file_path = storage.objects.name
        and public.can_view_team(auth.uid(), d.team_id)
    )
  )
);
