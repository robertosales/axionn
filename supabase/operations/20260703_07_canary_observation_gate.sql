-- One-off production operation: canary observation gate.
-- Run manually in Lovable Cloud SQL Editor after a successful frontend canary
-- observation window with VITE_ORG_TENANCY_ENABLED=true.
...
SELECT pg_advisory_xact_lock(hashtext('axionn:20260703:07_canary_observation_gate'));
...
BEGIN
  ...
  IF v_missing IS NOT NULL THEN
    RAISE EXCEPTION 'Missing required canary-observation dependencies: %', v_missing;
  END IF;

  IF public.is_tenancy_enforced() THEN
    RAISE EXCEPTION 'Canary observation gate requires tenancy enforcement disabled.';
  END IF;
  ...
  IF v_readiness_affected_rows <> 0 THEN
    RAISE EXCEPTION 'Canary observation gate blocked: readiness report has % affected rows.', v_readiness_affected_rows;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Canary observation gate blocked: organization without owner/admin.';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Canary observation gate blocked: no platform_admin exists.';
  END IF;
  ...
  IF has_function_privilege('authenticated', 'public.set_tenancy_enforcement(boolean)', 'EXECUTE')
    ...
    RAISE EXCEPTION 'Canary observation gate blocked: client role can toggle tenancy enforcement.';
  END IF;
  ...
  IF has_function_privilege('anon', 'public.get_my_organizations_v2()', 'EXECUTE')
    ...
    RAISE EXCEPTION 'Canary observation gate blocked: anon can execute tenant-scoped RPCs.';
  END IF;
END;
$$;
...
SELECT
  ...
  policy_summary.contract_select_policies,
  policy_summary.relation_policies,
  policy_summary.relation_policies_do_not_query_contracts_directly,
  ...
  (
    ...
    AND policy_summary.contract_select_policies = 1
    AND policy_summary.relation_policies = 6
    AND policy_summary.relation_policies_do_not_query_contracts_directly
  ...
  ) AS canary_observation_gate_ok_enforcement_off
