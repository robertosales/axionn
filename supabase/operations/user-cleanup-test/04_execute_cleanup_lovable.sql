-- EXECUCAO FINAL PARA O SQL EDITOR DO LOVABLE - AMBIENTE DE TESTE.
-- Conjunto esperado: 70 usuarios.
-- Fingerprint esperado: 32ad083d26b41dd1506a9e2950ed672e.
-- Este arquivo termina em COMMIT somente depois de todas as validacoes.

begin;

-- TRAVA MANUAL: substitua PENDING_CONFIRMATION somente após backup e aprovação.
do $explicit_confirmation$
declare
  confirmation constant text := 'PENDING_CONFIRMATION';
begin
  if confirmation <> 'DELETE_70_NON_WHITELIST_USERS_FROM_AXIONN_TEST_2026_07_13' then
    raise exception 'ABORT: confirmação explícita ausente ou inválida';
  end if;
end $explicit_confirmation$;
...
-- Sem FK, nenhuma deleÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o automÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡tica: o operador deve revisar/corrigir antes.
do $block$
...
begin
  ...
  loop
    ...
    if n>0 then raise exception 'BLOCKER sem FK: %.%.% possui % vÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­nculo(s)',r.s,r.t,r.c,n; end if;
  end loop;
end $block$;
...
-- Nunca apagar diretamente do catÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡logo: isso pode deixar blobs fÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­sicos ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³rfÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£os.
...
-- PolÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­tica aprovada: exclusÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o explÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­cita de dados pessoais, histÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³ricos, logs e backups.
do $personal_delete$
declare r record; n bigint;
begin
...
-- Preserva entidades de negÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³cio compartilhadas, removendo somente a atribuiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o pessoal.
with changed as (
...
-- O trigger de tenancy valida a organizaÃ§Ã£o inteira mesmo quando apenas
-- created_by muda. Isolamos a tabela e suspendemos somente esse trigger durante
-- a alteraÃ§Ã£o de autoria; FKs e demais triggers continuam ativos.
...
-- ValidaÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âµes antes de permitir commit.
do $validate$
declare n bigint;
begin
  select count(*) into n from auth.users u
  where not exists(select 1 from cleanup_whitelist w where w.email=lower(trim(u.email)));
  if n<>0 then raise exception 'Ainda existem % usuÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡rios fora da whitelist',n; end if;
  select count(*) into n from cleanup_whitelist w
  where (select count(*) from auth.users u where lower(trim(u.email))=w.email)<>1;
  if n<>0 then raise exception 'Whitelist corrompida apÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³s deleÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o'; end if;
  select count(*) into n from public.profiles p left join auth.users u on u.id=p.user_id where u.id is null;
  if n<>0 then raise exception 'Existem % profiles ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³rfÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£os',n; end if;
end $validate$;
...
-- O resultado consolidado acima é o comprovante da execução final.
