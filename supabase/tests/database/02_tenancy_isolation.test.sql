...
-- O onboarding legado associa novos usuários a este contrato padrão.
-- A fixture existe apenas nesta transação e é removida pelo rollback.
...
select is(public.is_platform_admin(), false, 'organization owner A is not a platform administrator');
select is(public.is_organization_member('10000000-0000-0000-0000-000000000001'), true, 'user A is a member of organization A');
select is(public.is_organization_member('10000000-0000-0000-0000-000000000002'), false, 'user A is not a member of organization B');
select is(public.can_operate_organization('10000000-0000-0000-0000-000000000001'), true, 'user A can operate organization A');
select is(public.can_operate_organization('10000000-0000-0000-0000-000000000002'), false, 'user A cannot operate organization B');

select results_eq(
  $query$ select id from public.get_accessible_contracts_v2('10000000-0000-0000-0000-000000000001') order by id $query$,
  $expected$ values ('40000000-0000-0000-0000-000000000001'::uuid) $expected$,
  'user A lists only contracts from organization A'
);
select is_empty(
  $query$ select id from public.get_accessible_contracts_v2('10000000-0000-0000-0000-000000000002') $query$,
  'user A cannot list contracts from organization B'
);
select results_eq(
  $query$ select id from public.get_accessible_companies_v2('10000000-0000-0000-0000-000000000001') order by id $query$,
  $expected$ values ('30000000-0000-0000-0000-000000000001'::uuid) $expected$,
  'user A lists only companies from organization A'
);
select results_eq(
  $query$ select id from public.get_accessible_projects_v2('10000000-0000-0000-0000-000000000001', null::uuid) order by id $query$,
  $expected$ values ('60000000-0000-0000-0000-000000000001'::uuid) $expected$,
  'user A lists only projects from organization A'
);
select results_eq(
  $query$ select id from public.get_accessible_teams_v2('10000000-0000-0000-0000-000000000001') order by id $query$,
  $expected$ values ('50000000-0000-0000-0000-000000000001'::uuid) $expected$,
  'user A lists only teams from organization A'
);
select results_eq(
  $query$ select id from public.get_my_organizations_v2() order by id $query$,
  $expected$ values ('10000000-0000-0000-0000-000000000001'::uuid) $expected$,
...
select is(public.is_organization_member('10000000-0000-0000-0000-000000000002'), true, 'user B is a member of organization B');
select is(public.is_organization_member('10000000-0000-0000-0000-000000000001'), false, 'user B is not a member of organization A');
select results_eq(
  $query$ select id from public.get_accessible_contracts_v2('10000000-0000-0000-0000-000000000002') order by id $query$,
  $expected$ values ('40000000-0000-0000-0000-000000000002'::uuid) $expected$,
  'user B lists only contracts from organization B'
);
select is_empty(
  $query$ select id from public.get_accessible_contracts_v2('10000000-0000-0000-0000-000000000001') $query$,
  'user B cannot list contracts from organization A'
);
select results_eq(
  $query$ select id from public.get_accessible_projects_v2('10000000-0000-0000-0000-000000000002', null::uuid) order by id $query$,
  $expected$ values ('60000000-0000-0000-0000-000000000002'::uuid) $expected$,
...
select is(public.is_organization_member('10000000-0000-0000-0000-000000000003'), true, 'suspended organization user remains a member');
select is(public.can_read_organization('10000000-0000-0000-0000-000000000003'), true, 'suspended organization user keeps read access');
select is(public.can_operate_organization('10000000-0000-0000-0000-000000000003'), false, 'suspended organization user cannot operate resources');

-- Administrador da plataforma.
select pg_temp.authenticate_as('20000000-0000-0000-0000-000000000004');

select is(public.is_platform_admin(), true, 'platform administrator is recognized independently from organization roles');
select results_eq(
  $query$
    select id
...
select is(public.can_operate_organization('10000000-0000-0000-0000-000000000003'), true, 'platform administrator can support a suspended organization');

-- Integridade entre organizações.
select throws_ok(
  $sql$ insert into public.contract_teams (contract_id, team_id) values ('40000000-0000-0000-0000-000000000001', '50000000-0000-0000-0000-000000000002') $sql$,
  'P0001',
  'contract_team_organization_mismatch',
  'contract cannot be linked to a team from another organization'
);
select throws_ok(
  $sql$ insert into public.contract_room_teams (contract_id, team_id, room_type, is_active) values ('40000000-0000-0000-0000-000000000001', '50000000-0000-0000-0000-000000000002', 'agil', true) $sql$,
  'P0001',
...
select throws_ok(
  $sql$
    insert into public.projects (id, name, module_type, status, contract_id, team_id)
    values ('60000000-0000-0000-0000-000000000099', 'Cross Tenant Project', 'agile', 'active', '40000000-0000-0000-0000-000000000001', '50000000-0000-0000-0000-000000000002')
  $sql$,
  'P0001',
...
select is(public.is_tenancy_enforced(), true, 'tenancy enforcement can be enabled by the privileged database session');
select throws_ok(
  $sql$ insert into public.companies (id, name, status) values ('30000000-0000-0000-0000-000000000090', 'Company Without Organization', 'active') $sql$,
  'P0001',
...
select pg_temp.authenticate_as('20000000-0000-0000-0000-000000000001');
select throws_ok(
  $sql$ insert into public.companies (id, name, status, org_id) values ('30000000-0000-0000-0000-000000000091', 'Unauthorized Company B', 'active', '10000000-0000-0000-0000-000000000002') $sql$,
  'P0001',
  'organization_not_operational',
  'user A cannot create resources in organization B'
);
select lives_ok(
  $sql$ insert into public.companies (id, name, status, org_id) values ('30000000-0000-0000-0000-000000000092', 'Authorized Company A', 'active', '10000000-0000-0000-0000-000000000001') $sql$,
  'user A can create resources in its active organization'
);

select pg_temp.authenticate_as('20000000-0000-0000-0000-000000000003');
select throws_ok(
  $sql$ insert into public.companies (id, name, status, org_id) values ('30000000-0000-0000-0000-000000000093', 'Blocked Suspended Company', 'active', '10000000-0000-0000-0000-000000000003') $sql$,
  'P0001',
  'organization_not_operational',
...
select pg_temp.authenticate_as('20000000-0000-0000-0000-000000000004');
select lives_ok(
  $sql$ insert into public.companies (id, name, status, org_id) values ('30000000-0000-0000-0000-000000000094', 'Platform Support Company', 'active', '10000000-0000-0000-0000-000000000003') $sql$,
  'platform administrator can operate suspended organization resources'
);

select public.set_tenancy_enforcement(false);
select is(public.is_tenancy_enforced(), false, 'tenancy enforcement can be returned to audit mode');
